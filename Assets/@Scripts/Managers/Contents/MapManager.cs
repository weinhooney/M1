...
public class MapManager 
{
    ...
    void SpawnObjectsByData(GameObject map, string mapName, string tilemap = "Tilemap_Object")
    {
        ...
        for(int y = tm.cellBounds.yMax; y >= tm.cellBounds.yMin; --y)
        {
            for(int x = tm.cellBounds.xMin; x <= tm.cellBounds.xMax; ++x)
            {
                ...
                if(Define.EObjectType.Env == tile.objectType)
                {
    ...
    {
        if (false == CanGo(cellPos)) { return false; }
    ...
    {
        if (cellPos.x < MinX || MaxX < cellPos.x) { return false; }
        if (cellPos.y < MinY || MaxY < cellPos.y) { return false; }

        if(false == ignoreObjects)
        {
        ...
        if (ECellCollisionType.None == type) { return true; }

        if (ignoreSemiWall && ECellCollisionType.SemiWall == type) { return true; }
    ...
    public List<Vector3Int> FindPath(Vector3Int startCellPos, Vector3Int destCellPos, int maxDepth = 10)
    {
        ...
        while(0 < pq.Count)
        {
            ...
            foreach(Vector3Int delta in _delta)
            {
                ...
                // 갈 수 없는 장소이면 스킵
                if (false == CanGo(next)) { continue; }

                // 예약진행
                int h = (dest - next).sqrMagnitude;

                // 더 좋은 후보 찾았는지
                if(false == best.ContainsKey(next))
                {
                    best[next] = int.MaxValue;
                }

                if (best[next] <= h) { continue; }
        ...
        // 제일 까까운 애라도 찾음
        if (false == parent.ContainsKey(dest)) { return CalcCellPathFromParent(parent, closestCellPos); }
    ...
    List<Vector3Int> CalcCellPathFromParent(Dictionary<Vector3Int, Vector3Int> parent, Vector3Int dest)
    {
        ...
        if (false == parent.ContainsKey(dest)) { return cells; }
